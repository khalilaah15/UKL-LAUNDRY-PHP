...
if($_POST){
    ...
    if(empty($nama_member)){
        ...
        include "koneksi.php";
        if(empty($alamat)){
            $update=mysqli_query($conn,"update member set nama_member='".$nama_member."',alamat='".$alamat."', jenis_kelamin='".$jenis_kelamin."', telepon='".$telepon."', kota='".$kota."' where id_member = '".$id_member."' ") or die(mysqli_error($conn));
            if($update){
